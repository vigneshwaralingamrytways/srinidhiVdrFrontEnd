...
export default function DocumentDetail({ user, onLogout }) {
    ...
    const fetchDownloadHistory = async (index) => {
        ...
        const reportDocId = rowData?.reportDocId;
        ...
        try {
            ...
            updated[index].downloadHistory = (res || []).map((h) => ({
                user: h.userName || authCtx.userName || "Unknown User",
    ...
    const handleDownload = async (index) => {
        const rowData = records[index];
        try {
            const val = { reportDocId: rowData.reportDocId };
            ...
            if (response.ok) {
                ...
                await post(api + "/downloadHistory/create", historyPayload);
                setRecords((prevRecords) => {
                    const updated = [...prevRecords];
                    const currentRecord = { ...updated[index] };
                    ...
                    currentRecord.downloadHistory = [newHistoryEntry, ...currentRecord.downloadHistory];
                    updated[index] = currentRecord;
                    return [...updated];
                });
    ...
    return (
        <div style={styles.page}>
            ...
            <div style={styles.body}>
                <div style={styles.backBtn} onClick={() => history.push("/document")}>
                    <FaArrowLeft /> Back
                </div>
                    ...
                    {loading ? (
                        ...
                        <>
                            <div className="table-scroll-wrapper" style={styles.tableScrollWrapper}>
                                <GlassTable
                                    headers={["Doc Name", "Remarks", "View", "Comment", "Download"]}
